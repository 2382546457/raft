...
public class ProtobufSerializer implements CustomSerializer {
    ...
    public <T extends RequestCommand> boolean serializeHeader(T request, InvokeContext invokeContext) throws SerializationException {
        ...
        final Message message = (Message) cmd.getRequestObject();
        if (message instanceof RpcRequests.AppendEntriesRequest) {
            final RpcRequests.AppendEntriesRequest req = (RpcRequests.AppendEntriesRequest) message;
            RpcRequests.AppendEntriesRequest.Builder builder = RpcRequests.AppendEntriesRequest
                    .newBuilder()
                    .setGroupId(req.getGroupId())
                    .setPeerId(req.getPeerId())
                    .setServerId(req.getServerId());
            cmd.setHeader(builder.build().toByteArray());
            return true;
        }
        return false;
    }
